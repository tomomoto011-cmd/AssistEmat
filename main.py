# =========================================================
#  ASSISTEMPAT BOT v4.5-fix (Health Mode + Fixed Syntax)
#  Исправления:
#  1. ✅ Вернул нормальное форматирование функций (исправлен SyntaxError)
#  2. ✅ Курс валют доступен через команду /currency (кнопка заменена на Здоровье)
#  3. ✅ Режим здоровья с изолированным контекстом
#  4. ✅ Строгие правила: ЗОЖ, без препаратов, дисклеймер
    ...
    async with db_pool.acquire() as conn:
        ...
        for sql in indexes:
            try:
                ...
                pass
    logging.info("✅ PostgreSQL initialized + v4.5-fix features")
...
async def get_tasks(uid, status="pending", category=None, parent_id=None, due_date_range=None):
    async with db_pool.acquire() as conn:
        ...
        if due_date_range:
            if not category and parent_id is None:
                query += " AND due_date >= $3 AND due_date <= $4"
            else:
                query += " AND due_date >= $4 AND due_date <= $5"
...
async def get_calendar_events(uid, from_date=None, to_date=None, category=None, limit=10):
    async with db_pool.acquire() as conn:
        ...
        if category:
            ...
            params.append(category)
        query += f" ORDER BY event_date ASC LIMIT ${len(params)+1}"
        params.append(limit)
        return await conn.fetch(query, *params)
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                ...
                json={
                    ...
                    "temperature": 0.9 if is_short else 0.7,
                    "max_tokens": 150 if is_short else 500
                }
            )
            r.raise_for_status()
...
Настроение пользователя учитывай, но фокус на практической пользе."""
    ...
    try:
        ...
        return "🩺 Я не могу дать медицинский совет. Рекомендую обратиться к врачу или фармацевту."

def get_fallback_response(user_text: str, mood: str) -> str:
    if mood == "грусть":
        return "Понимаю. Расскажи, что случилось? 🤍"
    if mood == "тревога":
        return "Всё будет хорошо. Что беспокоит?"
    if mood == "усталость":
        return "Отдохни. Я тут, если что. 🫂"
    return "Понял. 👍"
...
async def chat(msg: Message, state: FSMContext):
    ...
    if cmd:
        if cmd == "reset_context":
            ...
            return
        elif cmd == "ext_currency":  # 🔥 Курс через команду /currency
...
async def health_handler(request):
    return web.json_response({"status":"ok","bot":"AssistEmpat v4.5-fix"}, headers={"Content-Type":"application/json"})
...
async def main():
    logging.info(f"🚀 Starting AssistEmpat v4.5-fix (port={HEALTH_PORT}, TZ=Moscow)")
    ...
    logging.info("✅ AssistEmpat v4.5-fix ready — STARTING POLLING")
